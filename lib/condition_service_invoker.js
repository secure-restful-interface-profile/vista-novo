  ...
  restler.get(requestUrl, {headers: {'Authorization': req.header['authorization']}}).once('complete', function(vistaCondition, response) {
    ...
    if (vistaCondition instanceof Error) {
      ...
      if (response.statusCode == 200) {
        fhirize(req, res, vistaCondition, conditionId);
        next();        
      } else {
...
checkCollection = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.header['authorization']}}).once('complete', function(vistaConditions, response) {
    console.log("----- response.statusCode = " + response.statusCode + " -----");
    if (vistaConditions instanceof Error) {
      ...
      if (response.statusCode == 200) {
        vistaConditions.forEach(function(vistaCondition) {
          ...
          fhirize(req, res, vistaCondition, vistaCondition.id);
        });
        next();
      } else {
        // return error from backend service
...
fhirize = function(req, res, vistaCondition, id) {
