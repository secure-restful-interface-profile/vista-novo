    ...
    if (typeof req.params.id !== "undefined") {
      // ResourceHistory.findInCacheOrLocal(patientId, 'Patient', function(resourceHistory) {
        ...
        // } else {
          ...
          checkPatientObject(req, res, next);
        // }
      // });
    } else {
      checkPatientCollection(req, res, next);
    }
  } else {
    next();
  }
...
checkPatientObject = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.header['authorization']}}).once('complete', function(vistaPatient, response) {
    if (vistaPatient instanceof Error) {
      ...
      if (response.statusCode == 200) {
        fhirizePatient(req, res, vistaPatient, patientId, next);
        next();
      } else {
...
checkPatientCollection = function(req, res, next) {
    ...
    if (vistaPatients instanceof Error) {
      ...
      if (response.statusCode == 200) {
        vistaPatients.forEach(function(vistaPatient) {
          console.log("----- Patient -----");
          fhirizePatient(req, res, vistaPatient, vistaPatient.id, next);
        });
        next();
      } else {
        // return error from backend service
...
fhirizePatient = function(req, res, vistaPatient, id, next) {
