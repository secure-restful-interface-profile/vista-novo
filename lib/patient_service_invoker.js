...
exports.checkPatientCache = function(req, res, next, model) {
  ...
  if (model === 'patient') {
    if (typeof req.params.id !== "undefined") {
      checkPatient(req, res, next);
    } else {
      checkPatientList(req, res, next);
    }
  } else {
    next();
  }
...
checkPatient = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.headers['authorization']}}).once('complete', function(vistaPatient, response) {
    // console.log("  ----- response = " + util.inspect(response) + " -----");
    console.log("  ----- response.statusCode = " + response.statusCode + " -----");
    if (response.statusCode == 200) {
      lookupPatient(req, res, vistaPatient, patientId);
      next();
    } else {
...
checkPatientList = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.headers['authorization']}}).once('complete', function(vistaPatients, response) {
    // console.log("  ----- response = " + util.inspect(response) + " -----");
    console.log("  ----- response.statusCode = " + response.statusCode + " -----");
    if (response.statusCode == 200) {
      vistaPatients.forEach(function(vistaPatient) {
        console.log("  ----- Patient -----");
        lookupPatient(req, res, vistaPatient, vistaPatient.id);
      }); 
      next();
    } else {
      // return error from backend service
...
lookupPatient = function(req, res, vistaPatient, id) {
  ...
  ResourceHistory.findInCacheOrLocal(id, 'Patient', function(resourceHistory) {
    if (resourceHistory) {
      ...
      cacheFhirPatient(req, res, vistaPatient, id);
    }
  });
}
...
cacheFhirPatient = function(req, res, vistaPatient, id) {
