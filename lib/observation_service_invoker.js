...
checkObservation = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.headers['authorization']}}).once('complete', function(vistaObservation, response) {
...
checkObservationList = function(req, res, next) {
  ...
  restler.get(requestUrl, {headers: {'Authorization': req.headers['authorization']}}).once('complete', function(vistaObservations, response) {
    // console.log("  ----- response = " + util.inspect(response) + " -----");
    console.log("  ----- response.statusCode = " + response.statusCode + " -----");
    if (response.statusCode == 200) {
...
lookupObservation = function(req, res, vistaObservation, id) {
...
cacheFhirObservation = function(req, res, vistaObservation, id) {
  ...
  // Currently assumes blood pressure observation (for now)
